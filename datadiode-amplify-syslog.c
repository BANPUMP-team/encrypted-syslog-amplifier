...
int main(void)
{
	...
	string_to_blocks(buf, numbytes, pt_blocks, &num_blocks);
	   ...
	   // Encrypt each block but first 2 blocks are the plaintext counter
		 ...
		 * packets will always arrive in the correct order so it1 and it2 SpeckR knobs are ok 
		 *
		 * for async packets that can arrive in shuffled order it is mandatory to encrypt the same
		 * amount of blocks regardless of their arrival order in order to keep the dynamic Sboxes
		 * consistent between the sender and the receiver
		 *
		 * a data-diode with only one sender and only one receiver does not have this problem so 
		 * we can encrypt only the necessary amount of blocks and wire them amplified (repeated)
		 */
	...
	for (i=0; i<AMPFACTOR; i++) {
		if ((numbytes = sendto(sockfdout, ct_blocks, sizeof(uint32_t)*(num_blocks+2), 0, p->ai_addr, p->ai_addrlen)) == -1) { // send AMPFACTOR times
